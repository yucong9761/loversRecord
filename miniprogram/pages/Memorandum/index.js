Page({
  ...
  async toDetailPage(element, isUpper) {
    ...
    wx.navigateTo({url: '../../packageB/pages/MemorandumDetail/index?id=' + remark._id})
  },
  //转到任务详情[上]
  ...
  async toAddPage() {
    wx.requestSubscribeMessage({
      tmplIds: [ '36gzxjuEuUhaRlPfW4y_ZcmLf8-tOO5tJCKZ0I3a3ik'],
      success(res) {
        console.log('授权成功')
      },
      fail (err) {
    ...
    // await wx.cloud.callFunction({name: 'sendMessage', data: this.data}).then(
    //   () => {
    //       wx.showToast({
    //           title: '添加成功',
    //           icon: 'success',
    ...
    //   }
    // )
    // await wx.cloud.callFunction({name: 'pushNotify'})
    wx.navigateTo({url: '../../packageB/pages/MemorandumAdd/index'})
  },
